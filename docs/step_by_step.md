...
`shared_layers (Optional[Iterable[Iterable[str]]])`  defines groups of layers that share one quantization scheme; settable from the API and the CLI (`--shared_layers`). Under streaming mapped placement, the declared groups also stay on one device (placement atomicity only; joint scale search remains the layer_config comma-key mechanism).
...
- `--stream_prefetch off|auto|on|cpu|<device>` (default `off`) stages the next block's weights on another GPU or host RAM while the current block is quantized, slightly reducing total time -- the finished block's pack and shard write also overlap with the next block's tuning. With no multi-device `--device_map`, `auto` rotates blocks across the quant device and the next visible GPU.
- MTP/nextn tensors are passed through unquantized unless a `layer_config` pin covers them; GGUF exports store unpinned MTP tensors at the source precision (bf16 -> BF16), matching the other export formats. Pinning bits for the MTP layers (e.g. `".*mtp.*": {"bits": 8}`) quantizes the draft head for a smaller speculative-decoding draft.
...
- Without mapped placement, a block and its tuning state live on one GPU: with `iters > 0`, tuning state is several times the block's weight size, so a block that fits for RTN can still exceed one GPU under tuning.
- A single decoder block that does not fit one GPU can be distributed across several with a mapped placement: give `--device_map` a module template (e.g. `--device_map 'self_attn.*:0,mlp.*:1'` matches leaf names inside each block; repeated containers such as MoE experts stay whole on one device) or a comma device list (e.g. `--device_map 0,1`) for an automatic split: the placement follows the modules' observed execution order from each block layout's first reference forward and balances predicted peak VRAM per device -- weights plus tuning state (several times the weight size for linear layers; at `iters=0` this over-reserves, which is the safe direction) -- with headroom reserved for routed-expert activation buffers; repeated containers such as MoE experts and shared-quantization groups (`--shared_layers` entries) stay whole on one device, device boundaries fall on the cheapest activation cuts, and the placement is cached per layout so identical blocks reuse it. Each module's weights stream directly to its mapped device and tuning state (values, gradients) stays with its module, so per-GPU VRAM is roughly the block's state divided by the map width. Calibration rows are kept in host RAM in mapped mode. `--stream_prefetch_device_map` applies an alternate map to the prefetched (every other staged) blocks -- e.g. `--device_map 0,1 --stream_prefetch_device_map 2,3` splits a 4-GPU pool into two disjoint halves, so tuning and staging never share a device; the prefetched block is quantized in place on that map. It requires `--stream_prefetch` (and streaming); without it the flag is rejected. Mapped placement requires `--stream_quantization`, runs single-process, and cannot be combined with distributed tuning, `--stream_prefetch on`, or an explicit `--stream_prefetch` device (only `off`/`auto`/`cpu` remain valid). Background packing is not yet wired for mapped placement and runs serially.
