...
### AR_NEUQI_BACKEND
- **Description**: Backend chain for the NeUQI zero-point sweep. `"auto"` (default) keeps the reference chunked eager sweep on CPU and, on CUDA, serves the batched sweep with the hand-written Triton kernel from `auto_round_extension.triton.neuqi_sweep` (registers-resident: each weight element is loaded once and every (candidate, zero-point) loss is computed from registers), falling back to the `torch.compile`-fused sweep when Triton is unavailable. `"triton"` forces the Triton kernel, `"compile"` forces the `torch.compile` sweeps on any device, and `"eager"` always uses the reference sweep. All backends evaluate the exact same losses over the exact same candidate grid (selections identical up to near-ties: <~0.1% of groups flip, worst observed relative loss difference ~5e-5 on an RTX 3090, with the Triton kernel matching that tie profile exactly) and remove the HBM-traffic bottleneck of the brute-force grid on large batched expert searches (measured on a 2M-group, g=128, 64+32-candidate RTX 3090 sweep: 23.8 s eager -> 0.43 s compiled-batched -> 0.33 s Triton, ~72x). The per-candidate kernel computes rounding with native `cvt.rni.f32.f32` (measured selection-identical to the previous arithmetic emulation), statically unrolls the zero-point loop for bits <= 5, and skips the tail-group mask at pow2 group sizes. On top of it, the COARSE stage of the joint (scale, zero-point) search runs through a dedicated shared-multiplier kernel in normalized space (`neuqi_shared_sweep_triton`: the driver computes dn = w/s0 once; every group multiplies by the same 1/frac scalars -- no [C, K] scales tensor, no division in the candidate loop, argmin over (k, z) in-kernel; 0.32 s on the same sweep), engaged under `auto`/`triton` together with `AR_NEUQI_BATCH` and latching down to the batched path on any failure. Every stage latches down permanently on failure: shared Triton -> Triton -> compiled batched -> compiled per-candidate -> eager. Requires `AR_NEUQI_BATCH` to be enabled for the Triton/compiled-batched stages (it is by default). The same chain governs the symmetric two-stage search (`opt_rtn_int_sym_neuqi`), split by stage: under `auto` on CUDA the COARSE stage runs through the dedicated shared-multiplier Triton kernel `sym_search_shared_triton` (the driver normalizes once with dn = w/s0; every group then multiplies by the same 1/frac scalars, so the kernel does no division and no per-(group, candidate) scales exist -- measured RTX 3090, 2M groups, g=128, 256+64 candidates: 0.24 s vs 0.57-0.79 s for the compiled core and 8.5 s eager, with parity staying in the tie-flip class: ~0.5% flipped groups, max fp64 rel diff 1.7e-6 vs eager), while the FINE stage (per-group candidate grids) stays on the `torch.compile`-fused core. `triton` additionally forces the per-candidate `sym_search_triton` kernel for the fine stage, `compile` forces the compiled core on any device for both stages, and `eager` keeps the reference path. Every fallback latches down permanently on failure (e.g. machines without a C++ compiler for inductor codegen).
- **Default**: `"auto"`
...
- **Valid Values**: `minmax_qon` (explicit control arm), `neuqi_qon`, `neuqi_frozen_qon`, `neuqi_fp` (BPT/qoff-compatible), `opt_rtn_qon` (symmetric only), `neuqi_it0` (zero-shot reference marker; requires `--iters 0`)
...
### AR_TUNE_CUDA_GRAPHS (serial tune path)
- **Description**: When `AR_TUNE_CUDA_GRAPHS` is enabled and the tune loop runs serially (single device, no DDP, list inputs on a non-diffusion path, `gradient_accumulate_steps=1`, fence-free prepare active, `valid_token_mask` unused, non-LFQ block, loss on the tune device, and a torch build with CUDA-graph support — an unsupported build quietly keeps the legacy loop), each block's forward+loss+backward is captured into a CUDA graph after 2 warmup eager iterations: per-iteration sample gathers refresh STATIC device buffers (`_copy_into_static`, shape/dtype drift halts) and the iteration body becomes a single graph replay (warmup iterations run eagerly on the capture side stream so autograd nodes are born on the capturing stream). Grads stay parked (`set_to_none=False`) so backward accumulates into the addresses baked at capture; the optimizer step and best-params snapshot stay eager. When the pure-sign configuration holds (no scaler, momentum 0, no dynamic early-stop) and the device can spare one extra tuning-param set (VRAM-guarded), the OPTIMIZER STEP is captured inside the graph too (whole-iteration capture): the lr is read from a per-group device tensor refreshed each iteration (schedules that vary lr stay exact), and the best-params snapshot reads an in-graph PRE-STEP staging so the (loss, pre-step params) pairing is preserved bit-exactly. Capture defers while background CUDA threads (bg pack, checkpoint prefetch) are still active and logs once. Any capture/replay failure logs an error with the `AR_TUNE_CUDA_GRAPHS=0` hint and halts the run. Disqualified or CPU-only runs keep the legacy serial batch loop. See also the DDP variant under `AR_TUNE_EXCH_OVERLAP` constraints (eager backward only).
- **Default**: `1`
- **Valid Values**: `1`, `0`
- **Usage**: Keep enabled on CUDA; toggle off to isolate graph effects in A/B runs.
...
- **Description**: How many distinct layer-module templates keep their tune-loop graph state cached ACROSS blocks (persistent REPLICA SET — a HOME STAGING replica plus the mirrors — with static input buffers and the CAPTURED replica graphs). A same-template block (identical module structure, shapes AND per-module quant config + calibration geometry — a mixed-bits or config-changed run never false-hits) syncs its values into the cached replica set and the loop tunes the staging set: every captured graph replays (no warmup iterations, no capture on hits), snapshots read the staging home, and the final iterate is copied back into the real block so downstream readers see an in-place-tuned state. Saves the mirror build (~1.4-3.9 s), the capture (~1-3 s) and the warmup iterations per hit, and stops the per-block graph-pool churn that ratchets the reserved-VRAM watermark. `0` disables (per-block build+capture); `1` covers single-template runs; `2` covers a linear:full interleave; larger values fit models with more templates. The cap is enforced PER DDP GROUP (ping-pong staging: each group keeps its own newest N entries), so one group's template diversity never evicts the other's working set and the default stays right for any group count.
- **Default**: `2`
