...
- 算法与选项：支持 RTN 系列（普通 RTN 与优化版 RTN）和 SignRound（含 `iters > 0` 调优）。流式模式下
  SignRound 调优消费的是派生校准链——隐状态逐块传递，不存在整模型前向。这两个家族之外的量化器会在
  启动时被拒绝：它们的现有实现通过驱动完整物化模型来采集校准统计（激活、Hessian），尚未适配流式循环
  的逐块重放接口。这是集成缺口而非算法限制——逐层采集（如 GPTQModel 顺序 looper：钩住单层、重放其
  输入、把输出传给下一层）与校准链提供的"行进-行出"形态完全吻合，适配属于可行的后续工作。
  `enable_lfq` 同样被拒绝（其损失需要前向最后一块上仍是 meta 状态的 `lm_head`），扩散模型管道也被
  拒绝（没有可流式处理的逐块解码器列表）。注意 `auto_gptq`/`auto_awq` 作为导出格式仍然受支持——
  限制针对量化算法，而非打包布局。
...
- 支持的格式：流式量化要求逐块可打包的整数格式（`auto_round`、`auto_round:llm_compressor`、
  ...
  容器格式，只有当所有块都处理完毕后才能确定其张量目录，因此流式 GGUF 运行会把逐块打包好的 ggml
  载荷连同清单写入 `gguf-blobs/` 分片，保存时再从这些字节直接装配出最终 `.gguf`——纯装配、无二次量化，
  数值与非流式 GGUF 路径完全一致。模型自带 MTP/nextn 块时会保留嵌入；多模态投影器会写入单独的
  `mmproj-model.gguf`。
