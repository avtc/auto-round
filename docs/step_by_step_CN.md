...
- 算法与选项：支持 RTN 系列（普通 RTN 与优化版 RTN）和 SignRound（含 `iters > 0` 调优）。流式模式下
  SignRound 调优消费的是派生校准链——隐状态逐块传递，不存在整模型前向。而自身校准步骤需要驱动完整
  物化模型（激活统计/Hessian 采集，如 AWQ/GPTQ 风格量化器）的量化算法会在启动时被拒绝：meta 骨架同一
  时刻只驻留一个块，这类前向无从运行。`enable_lfq` 同样被拒绝（其损失需要前向最后一块上仍是 meta
  状态的 `lm_head`），扩散模型管道也被拒绝（没有可流式处理的逐块解码器列表）。注意 `auto_gptq`/
  `auto_awq` 作为导出格式仍然受支持——限制针对量化算法，而非打包布局。
