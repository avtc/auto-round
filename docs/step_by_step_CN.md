...
对于 K 块 GPU，利用率上界从 1/K 提升到 N/(N+K-1)（实际加速取决于 block 的设备分布与
Python 侧开销）。各微批损失的梯度之和在浮点归约误差范围内等于整批梯度，且优化器
仍然每次迭代只更新一次。标定收集阶段的多个分块前向在同一
标志下按分块粒度流水执行。切分只发生在样本维度——绝不切分 token 维度。大于 batch size
的取值会被截断到 batch size；该标志默认关闭。若当前配方使用会丢弃全批次顶部离群元素的自监督损失（SignRoundV2
低比特/激活量化的离群抑制损失），则会保持串行并给出一次性警告，因为按切片加权的均值无法复现该损失。对于带大型专家循环的 MoE block，N=4
