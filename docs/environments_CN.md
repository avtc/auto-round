...
- **描述**：控制分块（窗口化）反向传播，用于显存紧张的分块调优。普通调优路径会为所有被包裹的权重一次性构建 autograd 计算图；分块反向传播按行分块遍历每个权重，每块持有一个小计算图，产生逐位一致的梯度，但只保留一个块的中间量。`auto` 从普通路径开始调优，在发生显存不足（OOM）后切换为分块反向传播直至该块调优结束，随后从干净的梯度重启该迭代的全部子批次。`1` 从第一次迭代起强制使用分块反向传播——适用于已知显存紧张、不希望反复尝试的场景。`0` 完全禁用；OOM 将作为硬错误抛出。
- **默认值**：`auto`
- **合法取值**：`auto`、`1`、`0`
