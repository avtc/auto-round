...
### AR_MEM_COUNTERS
- **描述**：在流式循环中输出逐块 `[stream-mem]` 内存诊断（宿主内存 RSS 与逐 GPU 显存占用分布，归因最大的内存区域/张量，并给出峰值 RSS 归因），同时启用 DEBUG 级 `[vram]` 显存清点（分配器统计与最大的存活张量），覆盖调优开始与 block 外层入口。CUDA OOM 时附带的 WARNING 级清点始终输出。仅用于观察，不影响量化行为。
- **默认值**：`false`
