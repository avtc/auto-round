...
### AR_PRESINQ_BACKEND
- **描述**：Pre-SINQ 变换的 Sinkhorn 循环后端。`"auto"`（默认）在非 CPU 设备上将 Sinkhorn 迭代循环作为融合的 `torch.compile` 图运行 —— 每次迭代一个融合内核，而不是约 25 次内核启动，对 MoE 模型的每个专家的小型折叠收益最大 —— 在 CPU 上使用参考 eager 循环；`"compile"` 在任意设备上强制使用编译循环；`"eager"` 始终使用参考循环。两侧计算完全相同的 fp64 数学：eager 循环与上一版本位级一致（每次迭代的 std 只计算一次并复用，同时跳过不需要的缩放后矩阵物化 —— 本身就快约 1.8 倍），编译循环与其一致性约 1e-15（fp64 归约顺序效应；历史最优追踪器中的并列可能解析到另一个同等平衡的迭代）。任何编译或运行时失败都会在本次运行的剩余部分永久回退到 eager 循环。
- **默认值**：`"auto"`
- **有效值**：`"auto"`、`"compile"`、`"eager"`（无法识别的值按 `"eager"` 处理）
- **用法**：强制使用参考循环做 A/B 对比，或在 CPU 上强制融合以做实验
