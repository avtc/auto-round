...
- **注意**：`DEBUG` 还会激活流式量化内存诊断（逐 block 的 `[stream-mem]` inventory 分桶、大张量/内存区域归因，以及按流水线阶段归因峰值 RSS 的 peak watcher）
...
- **描述**: 仅影响调优类运行(`--iters > 0`)。此类运行通常会跳过仅存在于 checkpoint 中的分组(例如建模代码不会实例化的多 token 预测层)。例外:成功物化为真实模块树的分组(其张量能完整覆盖某个解码层兄弟块)会加入调优运行 —— 调优步骤会从校准链合成预测器输入,并用与整体模型相同的量化器配置对其调优。对于没有覆盖兄弟块的分组,本变量强制回退:被 pin 的仅 checkpoint 层仍在调优运行内部以闭式搜索(零样本路径)完成量化,而不是保持原样。模型其余部分不受影响。
- **用法**: 当被 pin 的仅 checkpoint 分组无法构建模块树(没有覆盖兄弟块)但仍需在调优运行内部量化时,设为 `1`。
