...
### AR_TUNE_DDP_DEVICES
- **描述**: `--parallel_quantization` 调优的可选显式副本设备列表（逗号分隔，例如 `0,1,2,3`）。默认情况下，计划会从空闲显存足以容纳镜像的可见 CUDA 设备中选取；每个副本在其计划设备上持有完整的块镜像，并抽取互不重叠的校验数据分片，因此等效批量与串行运行的数据覆盖一致。对于模块跨越多块 GPU 的调优块，镜像改为设备映射模式：每个副本在自己的一组设备上复现该块的阶段布局（总共 world x K 块设备），并按设备进行显存定价；该变量在此 lane 上被忽略。
- **默认值**: 未设置 → 由计划推导
- **取值**: 逗号分隔的设备索引
