...
### AR_NEUQI_COARSE
- **描述**：非对称联合（scale，零点）NeUQI 搜索在精细加性细化之前探索的粗粒度（对数间隔）scale 候选数量。对称两阶段搜索也会读取它作为回退（见 `AR_NEUQI_SYM_COARSE`）。真实模型 KL A/B（Qwen3.8-27B、g128、iters0）实测 64/32 vs 128/64 为 0.02570 vs 0.02571 —— 默认值下网格对真实权重组已饱和；更密的网格以约线性搜索时间换不到可测的端到端收益。
- **默认值**：`"64"`
- **有效值**：任意正整数
...
### AR_NEUQI_FINE
- **描述**：非对称联合 NeUQI 搜索中每个粗粒度候选对应的精细（加性）scale 细化候选数量（也是对称搜索的回退；见 `AR_NEUQI_SYM_FINE`）。细搜阶段是最便宜的质量杠杆：它括住每组自己的赢家，细搜加倍带来的差距缩减是粗搜加倍的 3-5 倍，而成本仅约 10%（合成收敛扫描实测）。
- **默认值**：`"32"`
- **有效值**：任意正整数
...
### AR_NEUQI_SYM_COARSE
- **描述**：仅作用于对称两阶段 scale 搜索的粗粒度候选数。优先级：本变量，然后 `AR_NEUQI_COARSE`，最后内置默认值。默认 256 与所有实测对称结果使用的网格一致（历史上通过 `AR_NEUQI_COARSE` 钉定；该钉定方式继续有效）。
- **默认值**：`"256"`
- **有效值**：任意正整数
- **用法**：独立于非对称搜索钉定对称网格
...
### AR_NEUQI_SYM_FINE
- **描述**：仅作用于对称两阶段 scale 搜索的精细候选数。优先级：本变量，然后 `AR_NEUQI_FINE`，最后内置默认值 64（所有实测对称结果使用的网格）。
- **默认值**：`"64"`
- **有效值**：任意正整数
- **用法**：独立于非对称搜索钉定对称网格
...
### AR_NEUQI_BACKEND
- **描述**：NeUQI 零点扫描的后端链。`"auto"`（默认）在 CPU 上保持参考的分块 eager 扫描，在 CUDA 上优先用 `auto_round_extension.triton.neuqi_sweep` 中的手写 Triton 内核服务批量扫描（寄存器驻留：每个权重元素只加载一次，所有（候选，零点）损失都在寄存器中计算），Triton 不可用时回退到 `torch.compile` 融合扫描；`"triton"` 强制使用 Triton 内核；`"compile"` 在任意设备上强制使用 `torch.compile` 融合扫描；`"eager"` 始终使用参考扫描。所有后端在完全相同的候选网格上计算完全相同的损失（选择仅在近似并列时不同：少于约 0.1% 的组会翻转，RTX 3090 上实测最坏相对损失差约 5e-5，Triton 内核的并列特征与其完全一致），并消除大规模批量专家搜索中暴力网格的显存带宽瓶颈（200 万组、g=128、64+32 候选的 RTX 3090 扫描实测：eager 23.8 秒 → 编译批量 0.43 秒 → Triton 0.33 秒，约 72 倍）。逐候选内核的舍入改用原生 `cvt.rni.f32.f32`（实测选择与原算术模拟完全一致）、位数不超过 5 时静态展开零点循环、2 的幂组宽下跳过尾部掩码。在此之上，联合（scale，零点）搜索的粗搜阶段走专用的共享乘子归一化空间内核（`neuqi_shared_sweep_triton`：驱动侧做一次归一化 dn = w/s0；每组乘以相同的 1/frac 标量 —— 不存在 [C, K] scales 张量、候选循环内无除法、（k, z）的 argmin 在内核内完成；同一扫描下 0.32 秒），在 `auto`/`triton` 且 `AR_NEUQI_BATCH` 启用时接入，任何失败都永久回退到批量路径。每一级失败都会永久逐级回退：共享 Triton → Triton → 编译批量 → 编译逐候选 → eager。Triton/编译批量阶段需要启用 `AR_NEUQI_BATCH`（默认启用）。同一后端链也作用于对称两阶段搜索（`opt_rtn_int_sym_neuqi`），并按阶段拆分：CUDA 上 `auto` 的粗搜阶段走专用共享乘子 Triton 内核 `sym_search_shared_triton`（驱动侧做一次归一化 dn = w/s0；每组乘以相同的 1/frac 标量，内核内无除法、也不存在按（组，候选）的 scales 张量 -- RTX 3090 实测，200 万组、g=128、256+64 候选：0.24 秒 vs 编译核心 0.57-0.79 秒 vs eager 8.5 秒，奇偶性保持在对决翻转类：约 0.5% 组翻转、对 eager 的最大 fp64 相对差 1.7e-6）；细搜阶段（按组候选网格）保持 `torch.compile` 融合核心。`triton` 额外强制细搜阶段使用按候选的 `sym_search_triton` 内核；`compile` 在任意设备上对两个阶段都强制使用编译核心；`eager` 保持参考路径。每一级回退在失败时永久锁定（例如缺少 inductor 代码生成所需 C++ 编译器的机器）。
- **默认值**：`"auto"`
...
- **描述**： 跨 block 缓存多少种不同 layer-module 模板的调优图状态（持久 REPLICA 集合——HOME 暂存 replica 加各镜像——含静态输入缓冲与已捕获的 replica graphs）。同模板 block（模块结构、形状以及逐模块量化配置 + 校准几何完全一致——混合位数或配置变更的运行不会误命中）将自身数值同步进缓存 replica 集合，调优循环在暂存集合上进行：所有已捕获图全部 replay（命中时无预热迭代、无捕获），快照从暂存 home 读取，最终迭代值拷回真实 block 使下游看到与原地调优一致的状态。每次命中节省镜像构建（约 1.4-3.9 秒）、捕获（约 1-3 秒）与预热迭代，并消除逐 block 的图池churn（其会不断抬升 reserved-VRAM 高水位）。`0` 禁用（逐 block 构建+捕获）；`1` 覆盖单模板模型；`2` 覆盖线性:全注意力交替；更多模板的模型可调大。上限按 DDP GROUP 分别生效（ping-pong staging：每个 group 各自保留最新的 N 个条目），因此一个 group 的模板多样性不会驱逐另一个 group 的工作集，默认值对任意 group 数都正确。
- **默认值**: `2`
- **有效值**: 非负整数
