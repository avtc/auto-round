...
### AR_MEM_COUNTERS
- **描述**：在流式循环中输出逐块 `[stream-mem]` 内存诊断：宿主内存 RSS 与逐 GPU 显存占用分布，并归因最大的内存区域/张量，同时给出峰值 RSS 归因。仅用于观察，不影响量化行为。该开关仅作用于这些分支新增的日志行；上游的逐块 `Memory Monitor` 摘要不受此变量影响，保持上游行为。
- **默认值**：`false`
