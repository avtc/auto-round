...
AUTO_ROUND_PATH = str(Path(*_parts[: _parts.index("test")]))


def _run_cli(*args: str) -> int:
    """Invoke the auto_round CLI in a subprocess with the repo on PYTHONPATH."""
    import subprocess

    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join([AUTO_ROUND_PATH, env.get("PYTHONPATH", "")])
    return subprocess.run([sys.executable, "-m", "auto_round", *args], env=env).returncode
...
class TestAutoRoundCmd:
    ...
    def test_auto_round_cmd(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        assert _run_cli("-h") == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd2(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        ret = _run_cli(
            "--model", tiny_opt_model_path,
            "--seqlen", "32", "--iter", "2", "--nsamples", "1",
            "--format", "auto_gptq,auto_round",
            "--output_dir", self.save_dir,
            "--tasks", "piqa", "--limit", "2",
        )
        assert ret == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd3(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        ret = _run_cli(
            "--model", tiny_opt_model_path,
            "--seqlen", "8", "--iter", "1", "--nsamples", "1",
            "--eval_task_by_task", "--tasks", "openbookqa", "--bs", "32", "--limit", "2",
        )
        assert ret == 0, "cmd line test fail, please have a check"
    ...
    def test_auto_round_cmd4(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        ret = _run_cli(
            ...
            "--seqlen", "8", "--iter", "2", "--nsamples", "8",
            "--output_dir", self.save_dir,
            "--tasks", "lambada_openai", "--limit", "2",
        )
        assert ret == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd5(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        assert _run_cli("--eval", "-h") == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd6(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        assert _run_cli("--eval", "--lmms", "-h") == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd7(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        ret = _run_cli(
            "--mllm", "--model", tiny_qwen_vl_model_path,
            "--iter", "2", "--nsamples", "2", "--seqlen", "32",
            "--format", "auto_round", "--output_dir", self.save_dir,
        )
        assert ret == 0, "cmd line test fail, please have a check"

    def test_auto_round_cmd8(self, tiny_opt_model_path, tiny_qwen_vl_model_path):
        ret = _run_cli(
            "--mllm", "--iter", "2", "--nsamples", "2",
            "--model", tiny_qwen_vl_model_path, "--seqlen", "32",
            "--format", "auto_round", "--quant_nontext_module",
            "--output_dir", self.save_dir,
        )
        assert ret == 0, "cmd line test fail, please have a check"
    ...
    def test_layer_config(self, tiny_opt_model_path):
        ...
        ret = _run_cli(
            "--model", tiny_opt_model_path,
            "--seqlen", "8", "--iter", "0", "--disable_opt_rtn",
            "--layer_config", layer_cfg,
            "--format", "auto_round", "--output_dir", self.save_dir,
        )
        assert ret == 0, "cmd line test with --layer_config fail, please have a check"
