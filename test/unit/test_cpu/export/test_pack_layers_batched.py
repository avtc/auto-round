...
class _MoEish(nn.Module):
    def __init__(self, n_experts=4, out=32, inp=32, bias=False):
        super().__init__()
...
_PACKED_CLASS_NAMES = {"QuantLinear", "WQLinear_GEMM"}


def _packed(m, name):
    mod = get_module(m, name)
    assert type(mod).__name__ in _PACKED_CLASS_NAMES, f"{name} not packed"
    return mod


# plain auto_round (qlinear_torch), routed gptq (qlinear_torch_zp), routed awq
# (WQLinear_GEMM, 4-bit only) -- every batchable packer must be bitwise-identical
# to its own per-module pack_layer.
_BACKENDS = ["auto_round:exllamav2", "auto_round:auto_gptq", "auto_round:auto_awq"]


@pytest.mark.parametrize("backend", _BACKENDS)
@pytest.mark.parametrize("sym,zp_tensor,bias", [(True, False, False), (False, True, False), (False, False, True)])
def test_batched_matches_per_module(backend, sym, zp_tensor, bias):
    ...
    per_module_pack = pack_layer
    if backend == "auto_round:auto_awq":
        # the awq packer packs through its own per-module path in production
        from auto_round.export.export_to_awq.export import pack_layer as per_module_pack
    for n in names:
        per_module_pack(n, a, backend=backend, device="cpu")
    packed, leftover = pack_layers_batched(names, b, backend=backend, device="cpu")
...
def test_shape_not_multiple_of_32_falls_back():
    # The qweight/qzeros lane math (out // 32 * bits columns) is exact only for
    # multiples of 32; such groups must keep the per-module path instead of
    # producing misaligned slices.
    a = _MoEish(n_experts=2, out=20).arm()
    names = ["experts.0", "experts.1"]
    packed, leftover = pack_layers_batched(names, a, backend="auto_round:exllamav2", device="cpu")
    assert packed == []
    assert leftover == names
    for n in names:  # untouched: still plain Linear
        assert isinstance(get_module(a, n), nn.Linear)


@pytest.fixture
def export_log(caplog):
    """Capture warnings from the export module logger (propagate=False blinds caplog)."""
    import logging

    from auto_round.export.export_to_autoround import export as _exp

    _exp.logger.setLevel(logging.WARNING)
    _exp.logger.addHandler(caplog.handler)
    yield caplog
    _exp.logger.removeHandler(caplog.handler)


def test_heterogeneous_int_zero_points_fall_back(export_log):
    # Same-shape modules whose integer zero points differ cannot share one
    # pack call; they must fall back to per-module packing with a warning,
    # never raise.
    a = _MoEish(n_experts=2).arm(sym=False, zp_tensor=False)
    a.experts[1].zp = 5
    names = ["experts.0", "experts.1"]
    packed, leftover = pack_layers_batched(names, a, backend="auto_round:exllamav2", device="cpu")
    assert packed == []
    assert leftover == names
    assert "integer zero points differ" in export_log.text
    for n in names:
        assert isinstance(get_module(a, n), nn.Linear)


def test_mixed_int_and_tensor_zero_points_fall_back(export_log):
    a = _MoEish(n_experts=2).arm(sym=False, zp_tensor=True)
    a.experts[1].zp = 4
    names = ["experts.0", "experts.1"]
    packed, leftover = pack_layers_batched(names, a, backend="auto_round:exllamav2", device="cpu")
    assert packed == []
    assert leftover == names
    assert "mixed tensor and integer zero points" in export_log.text
