...
@pytest.fixture()
def capture_manifest(monkeypatch):
    """Capture the blob manifest content at discard time.

    A successful assembly removes the whole gguf-blobs/ directory (shards +
    manifest), so tests that inspect the manifest after a successful save must
    grab it while discard_shards can still see it."""
    import json as _json

    from auto_round.export.export_to_gguf.blob_store import GgufBlobStore

    captured: dict = {}
    original = GgufBlobStore.discard_shards

    def _spy(self):
        path = self._manifest_path()
        if path.is_file():
            captured["manifest"] = _json.loads(path.read_text())
        return original(self)

    monkeypatch.setattr(GgufBlobStore, "discard_shards", _spy)
    return captured
...
class TestBlobResumeAndRerun:
    ...
    def test_mtp_only_and_disable_envs_are_mutually_exclusive(self, monkeypatch):
        """Uses the real OS env (setenv/delenv), never monkeypatch.setattr on
        the lazy envs module: hasattr() fires __getattr__, so setattr would
        make monkeypatch believe the attribute pre-existed and teardown would
        write a permanent module global that shadows the lazy lambda for the
        rest of the process (leaks False into every later env read)."""
        from auto_round.export.export_to_gguf.export import _validate_gguf_mtp_envs

        monkeypatch.delenv("AR_DISABLE_GGUF_MTP_EXPORT", raising=False)
        monkeypatch.delenv("AR_GGUF_MTP_ONLY", raising=False)
        _validate_gguf_mtp_envs()  # neither set: fine

        monkeypatch.setenv("AR_DISABLE_GGUF_MTP_EXPORT", "1")
        _validate_gguf_mtp_envs()  # disable alone: fine

        monkeypatch.delenv("AR_DISABLE_GGUF_MTP_EXPORT")
        monkeypatch.setenv("AR_GGUF_MTP_ONLY", "1")
        _validate_gguf_mtp_envs()  # mtp-only alone: fine
...
class TestStreamGgufBlobMtpE2E:
    ...
    @pytest.mark.timeout(300)
    def test_nextn_pins_and_kv_in_assembled_gguf(self, capture_manifest, tiny_qwen3next_ckpt, tmp_path, monkeypatch):
        ...
        assert not (Path(ar.output_dir) / "gguf-blobs").exists(), "blob dir removed after success"
    ...
    def test_mtp_only_env_exports_separate_draft_gguf(
        self, capture_manifest, tiny_qwen3next_ckpt, tmp_path, monkeypatch
    ):
        """AR_GGUF_MTP_ONLY=1 exports ONLY the MTP/nextn predictor tensors as
        a separate mtp-*.gguf draft file (llama.cpp --model-draft workflow):
        no body decoder tensors, embed/norm/lm_head kept per the vendored
        filter, pins still reach the draft tensors."""
        gguf = pytest.importorskip("gguf")
        from auto_round.algorithms.quantization.rtn.config import RTNConfig
        from auto_round.autoround import AutoRound
        from auto_round.export.export_to_gguf import llama_cpp_conversion as lcc
        ...
        monkeypatch.setattr(conversion_qwen.Qwen3NextModel, "get_vocab_base_pre", lambda self, tokenizer: "gpt2")
        monkeypatch.setenv("AR_GGUF_MTP_ONLY", "1")
        # a previous in-process test cached its (non-mtp-only) conversion
        # instance for this fixture path; the draft export must start from
        # fresh conversion state or the cached filter settings leak in
        import auto_round.export.export_to_gguf.export as gguf_export

        gguf_export._clear_gguf_model_instances()
        GgufBlobStore.reset_singletons()
        out_dir = str(tmp_path / "out_mtp_only")
        ar = AutoRound(
            tiny_qwen3next_ckpt,
            scheme="gguf:q4_0",
            alg_configs=[RTNConfig(group_size=32, disable_opt_rtn=True)],
        ...
        entries = manifest["roles"]["text"]["tensors"]
        names = {e["name"] for e in entries}
        # the mtp-only filter keeps the MTP block + embed/final-norm/lm_head only
        assert not any(
            n.startswith(("blk.0.", "blk.1.")) for n in names
        ), f"body decoder tensors leaked into the mtp-only draft: {sorted(n for n in names if n.startswith(('blk.0.', 'blk.1.')))[:5]}"
        assert any(".nextn." in n for n in names), f"draft file carries no nextn tensors; names: {sorted(names)}"
        for want in ("token_embd.weight", "output_norm.weight"):
            assert want in names, f"{want} missing from the mtp-only draft (vendored keep-list)"

        out_path = Path(manifest["roles"]["text"]["out_path"])
        assert out_path.is_file(), f"assembled draft gguf missing at {out_path}"
        assert out_path.name.startswith("mtp-") and out_path.name.endswith(
            ".gguf"
        ), f"draft file must use the mtp-* naming convention, got {out_path.name}"

        reader = gguf.GGUFReader(str(out_path))
        dtypes = {t.name: t.tensor_type for t in reader.tensors}
        reader_names = set(dtypes)
        assert not any(n.startswith(("blk.0.", "blk.1.")) for n in reader_names), "body tensors in assembled draft"
        assert any(".nextn." in n for n in reader_names), "assembled draft carries no nextn tensors"
        assert (
            dtypes.get("blk.2.nextn.eh_proj.weight") == gguf.GGMLQuantizationType.Q8_0
        ), "mtp bits-8 pin did not reach the draft file"
        assert (
            dtypes.get("output.weight") == gguf.GGMLQuantizationType.Q8_0
        ), "lm_head bits-8 pin did not reach the draft file"
        kv_names = {str(k) for k in reader.fields}
        assert any("nextn_predict_layers" in k for k in kv_names), "nextn kv missing from the draft"
...
class TestStreamGgufBlobMmprojE2E:
    ...
    @pytest.mark.timeout(900)
    def test_stream_quantize_assembles_mmproj(self, capture_manifest, vl_ckpt, tmp_path, monkeypatch):
...
class TestDiscardShardsAfterAssembly:
    ...
    assembled the whole gguf-blobs/ directory (shards + manifest) is removed
    automatically -- a crashed run keeps everything for resume adoption."""

    def test_discard_shards_removes_the_blob_directory(self, store, tmp_path):
        conv = _FakeConversion(fname_out=str(tmp_path / "m.gguf"))
        rec = store.attach_recorder(conv, "text")
        _add_q4_tensor(rec, "blk.0.attn_q.weight", 4)
        store.flush("text")
        ...
        assert not store.blob_dir.exists(), "the whole gguf-blobs directory must be removed"
