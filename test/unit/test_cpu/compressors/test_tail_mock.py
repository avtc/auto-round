...
class TestPassthroughStub:
    def test_bare_arity_returns_input_unchanged(self):
        stub = PassthroughStub(tuple_arity=False)
        x = torch.randn(2, 3, 4)
        out = stub(x)
        assert out is x

    def test_tuple_arity_returns_single_element_tuple(self):
        stub = PassthroughStub(tuple_arity=True)
        x = torch.randn(2, 3, 4)
...
class TestTailInjector:
    def test_returns_cached_tail_regardless_of_input(self):
        ...
        inj = TailInjector(tail, tuple_arity=False)
    ...
    def test_set_tail_swaps_payload(self):
        inj = TailInjector(torch.randn(1, 2, 8), tuple_arity=False)
    ...
    def test_tuple_arity_matches_stub_convention(self):
        ...
        inj = TailInjector(tail, tuple_arity=True)
...
class TestInstallRestore:
    def test_install_replaces_all_block_slots_with_stubs(self):
        model = _fake_model()
        install_block_stubs_(model, block_names=["model.layers.0", "model.layers.1", "model.layers.2"])
    ...
    def test_restore_returns_original_modules_bit_identical(self):
        ...
        restore_info = install_block_stubs_(model, block_names=["model.layers.0", "model.layers.1", "model.layers.2"])
