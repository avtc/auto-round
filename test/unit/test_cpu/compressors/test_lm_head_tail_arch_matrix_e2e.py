...
class TestLmHeadTailArchMatrix:
    ...
    @pytest.mark.xfail(
        reason="residual-pair families: the block-replay chain tail's hidden stream differs "
        "from a genuine full forward by ~1e-2 (residual reconstruction upstream of the lm_head "
        "lane); the lane itself engages - smoke passes at arity 2 and the capture runs"
    )
    ...
    @pytest.mark.xfail(
        reason="upstream block-replay gap: cached block kwargs lack position_embeddings for "
        "deepseek_v4 (dies during BLOCK quantization, before the lm_head lane; unrelated to it)"
    )
