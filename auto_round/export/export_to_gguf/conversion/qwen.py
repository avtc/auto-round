...
class _QwenMtpMixin:
    ...
    @classmethod
    def _remap_mtp_name_(cls, name: str, base_block_count: int) -> str:
        """Rewrite an ``mtp.*`` tensor name to the layer-indexed nextn spelling.

        Shared by the checkpoint path (``filter_tensors``) and the live-model
        path (streaming exports read tensors from the in-memory state dict,
        where the MTP block keeps its ``mtp.*`` module names).
        """
        ...
        if len(parts) == 4 and parts[1] == "layers" and parts[2].isdecimal():
            mtp_idx = int(parts[2])
            name = f"model.layers.{base_block_count + mtp_idx}.{parts[3]}"
            cls.opt_num_mtp_layers = max(cls.opt_num_mtp_layers, mtp_idx + 1)
        elif len(parts) == 3 and parts[1] in cls._MTP_LEAF_REMAPPER:
            name = f"model.layers.{base_block_count}.{cls._MTP_LEAF_REMAPPER[parts[1]]}.{parts[2]}"
        return name
    ...
    def filter_tensors(cls, item):
        ...
        if not is_mtp and not cls.no_mtp:
            # Streams/walks may hand us the REMAPPED extended-block spelling
            # (model.layers.{base+k}....) instead of the raw mtp.* one; treat
            # layer indices at or beyond the decoder depth as MTP so the
            # mtp_only keep-list and no_mtp drop both see them correctly
            base = getattr(cls, "_original_block_count", None)
            if base is not None:
                parts = name.split(".")
                if parts and parts[0] == "model":
                    parts = parts[1:]
                if len(parts) >= 3 and parts[0] == "layers" and parts[1].isdecimal() and int(parts[1]) >= base:
                    is_mtp = True
