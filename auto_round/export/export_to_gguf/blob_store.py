...
class GgufBlobStore:
    ...
    def discard_shards(self) -> int:
        """Remove the whole gguf-blobs directory once every role is assembled.
        ...
        and the manifest has no consumer once the shards are gone. Only
        called after ALL roles assembled successfully - a crashed run keeps
        its shards and manifest for resume adoption.
        """
        removed = 0
        ...
        if self.blob_dir.exists():
            try:
                ...
                logger.warning("[gguf-blob] could not remove %s: %s", self.blob_dir, e)
        if removed:
            logger.info(
                "[gguf-blob] all roles assembled; removed %d blob shard file(s) and the blob directory", removed
            )
        return removed
