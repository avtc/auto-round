...
class OptimizedRTNQuantizer(RTNQuantizer):
    ...
    def _expert_search_active(self) -> bool:
        ...
        if bool(getattr(cfg, "disable_opt_rtn", False)):
            return False
        if getattr(cfg, "orig_disable_opt_rtn", None) is None and getattr(
            self.model_context, "is_moe_model", False
        ):
            return False
    ...
    def _quantize_expert_batch(self, mods: list, device) -> bool:
        ...
        for s in range(0, len(mods), per_call):
            ...
            weights = torch.cat(
                [m.weight.data.reshape(m.weight.shape[0], -1) for m in chunk], dim=0
            ).to(dev)
            imat_rows = []
            for m in chunk:
                if hasattr(m, "imatrix"):
    ...
    def _quantize_targets(self, targets: list) -> None:
        ...
        logger.info(
            "[OptRTN] tuning fan-out: %d jobs (%d batched expert groups, %d modules) "
            "across %d GPUs (round-robin).",
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as pool:
            ...
            for i, (kind, payload) in enumerate(jobs):
                ...
                if kind == "batch":
                    ...
                    futures.append(
                        pool.submit(
                            _timed, kind, name, self.quantize_layer_outside_block, payload, device_override=dev
                        )
                    )
            for f in futures:
        ...
        if phases:
            ...
            logger.info(
                ...
                elapsed, slowest, stack_t, search_t, write_t,
            )
        else:
