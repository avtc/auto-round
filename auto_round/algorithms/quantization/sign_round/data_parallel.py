...
def enable_peer_access(devices) -> list:
    ...
    if os.environ.get("AR_TUNE_DISABLE_P2P"):
        logger.info(
            "AR_TUNE_DISABLE_P2P=1: skipping P2P enablement; cross-device copies will stage through host memory"
        )
        return []
    ...
    through host memory (several GB/s instead of device-to-device rates),
    which showed up as a multi-x allreduce slowdown. Returns the list of
    enabled "i->j" pairs for logging.

    Fail-visible: every cudart return code is checked; failures (and the
    human-readable cuda error string) are logged as WARNINGGs. A pair whose
    enable call returned cudaErrorPeerAccessAlreadySet counts as enabled;
    anything else does not, so the "enabled for N pairs" line can never
    overstate what actually happened.
    """
    import ctypes

    _ALREADY_SET = 704  # cudaErrorPeerAccessAlreadySet (716 is misaligned address)

    def _describe(cudart, rc: int) -> str:
        try:
            return ctypes.c_char_p(cudart.cudaGetErrorString(rc)).value.decode(errors="replace")
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "[tune-ddp] P2P enable skipped: cannot load libcudart (%r); cross-device copies may "
    ...
    saved = torch.cuda.current_device() if torch.cuda.is_available() else None
    try:
        for i in devs:
            for j in devs:
                if i == j:
                    continue
                can = ctypes.c_int(0)
                rc = cudart.cudaDeviceCanAccessPeer(ctypes.byref(can), i, j)
                if rc != 0:
                    logger.warning("[tune-ddp] P2P probe failed %d->%d: %s", i, j, _describe(cudart, rc))
                    continue
                if not can.value:
                    continue  # topology cannot peer -- not an error
                rc = cudart.cudaSetDevice(i)
                if rc != 0:
                    logger.warning(
                        "[tune-ddp] P2P enable failed %d->%d: cudaSetDevice: %s", i, j, _describe(cudart, rc)
                    )
                    continue
                rc = cudart.cudaDeviceEnablePeerAccess(j, 0)
                if rc == 0 or rc == _ALREADY_SET:
                    enabled.append(f"{i}->{j}")
                else:
                    logger.warning(
                        "[tune-ddp] P2P enable failed %d->%d: %s; cross-device copies between this pair may "
                        "stage through host memory",
                        i,
                        j,
                        _describe(cudart, rc),
...
def resolve_tune_ddp_plan_(quantizer, block, fp_inputs, fp_outputs, home, world=None, log: bool = True):
    ...
    eligible = (
        world > 1
        and home.type == "cuda"
        and quantizer._get_scaler() is None
    ...
    if world > 1 and quantizer._get_scaler() is not None:
    ...
    if eligible:
        ...
        batch_size = getattr(quantizer.calibration_context, "batch_size", nsamples)
