...
_GRAPHS_SAMPLER_SKIP_LOGGED = False
...
def cuda_graphs_engage(replica_group, samplers_active: bool) -> bool:
    """Decide whether the DDP tune loop may capture per-replica CUDA graphs.

    Quiet refusals are environment properties only (serial path, shard
    sampler active, no CUDA). Anything failing *during* capture or replay is
    a real fault and raises (see :class:`GraphedReplicaStep`).
    """
    global _GRAPHS_SAMPLER_SKIP_LOGGED
    if replica_group is None:
        return False
    if samplers_active:
        if not _GRAPHS_SAMPLER_SKIP_LOGGED:
            _GRAPHS_SAMPLER_SKIP_LOGGED = True
            logger.info(
                "[tune-ddp] shard sampler active: cuda graphs skipped "
                "(a replay would keep reading the captured iteration's shard)"
            )
        return False
...
class GraphedReplicaStep:
    """Capture one replica's tune step (fwd + loss + backward) in a CUDA graph.

    Capture-safety contract for the wrapped callable:
    - every input tensor it reads lives at a stable address for the whole
      block (static shards, persistent index tensors, pooled device-local
      reference outputs);
    - no host synchronization inside (``.item()``/``.cpu()``) -- the loss is
      returned as a tensor and read later;
    - parameters are stepped in place by the eager optimizer and grads stay
      parked (``zero_grad(set_to_none=False)``) so backward accumulates into
      the addresses baked at capture time.

    The first ``warmup_iters`` calls run eagerly; the next call captures
    (recording only -- the kernels do not execute) and immediately replays
    once so that iteration's work actually happens; later calls replay and
    return a clone of the static loss tensor. Any capture/replay failure is
    logged and re-raised: the run halts instead of silently degrading --
    set AR_TUNE_CUDA_GRAPHS=0 to disable graphs.
    """

    def __init__(self, fn, *, warmup_iters: int = 2, name: str = "", graph_factory=None, capture_ctx=None):
        self.fn = fn
        ...
        if self._graph is None:
            ...
            try:
                ...
                with self._capture_ctx(graph):
                    self._static_loss = self.fn()
                # capture only RECORDS: execute this iteration's work now
                graph.replay()
                self._graph = graph
                ...
                raise
            logger.info(
                "[tune-ddp] cuda graph captured for %s after %d warmup iteration(s)",
                self.name,
                self.warmup_iters,
            )
        else:
